			...
			<table>
				...
				<tbody>
					<tr v-for="a in appeals" :key="a.id">
						...
						<td>
							<button v-if="a.status === 'ARBITRATING'" class="btn btn-warning btn-sm" @click="openArbitrateModal(a)">仲裁</button>
							<button class="btn-link" @click="viewDetail(a)">详情</button>
						</td>
					</tr>
					<tr v-if="appeals.length === 0">
						<td colspan="9" class="empty-text">暂无仲裁中申诉</td>
					</tr>
				</tbody>
			</table>
		</div>
		...
		<div v-if="showArbitrateModal" class="modal-mask">
			<div class="modal-content">
				<div class="modal-header">
					<h3>仲裁 - {{ currentAppeal?.studentName }}</h3>
				...
				<form @submit.prevent="submitArbitrate">
					<div class="modal-body">
						...
						<div v-if="currentAppeal?.teacherComment" class="info-row">
							<span>教师评语: {{ currentAppeal?.teacherComment }}</span>
						</div>
						...
						<div v-if="arbitrateForm.decision === 'APPROVED'" class="form-group">
						...
						<div class="form-group">
							...
							<textarea v-model="arbitrateForm.comment" rows="3" required></textarea>
						</div>
					</div>
			...
			<div class="modal-content modal-lg">
				...
				<div class="modal-body">
					<div class="detail-grid">
						...
						<div class="detail-item full"><span class="label">管理员意见:</span> {{ detailData?.adminComment ?? '-' }}</div>
					</div>
...
import { getArbitratingAppeals, arbitrateAppeal, getAppealDetail, getAppealAuditLogs, type AppealDto, type AuditLogDto } from '@/api/appeal'
...
const arbitrateForm = ref({ decision: '', comment: '', finalScore: null as number | null })
...
const fetchAppeals = async () => {
	try {
		const res = await getArbitratingAppeals()
...
const openArbitrateModal = (a: AppealDto) => {
	...
	arbitrateForm.value = { decision: '', comment: '', finalScore: null }
...
const submitArbitrate = async () => {
	...
	try {
		await arbitrateAppeal(currentAppeal.value.id, {
			decision: arbitrateForm.value.decision,
			comment: arbitrateForm.value.comment,
			finalScore: arbitrateForm.value.decision === 'APPROVED' ? arbitrateForm.value.finalScore : undefined,
		})
